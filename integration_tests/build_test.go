...
`, baseImage))
	...
	`)
			...
			Expect(buildprobeData.BuilderBaseImages).To(HaveLen(1))
			Expect(buildprobeData.BuilderBaseImages[0].Pullspec).To(Equal(secondBase))
			Expect(buildprobeData.BaseImage.Pullspec).To(Equal(baseImage))
	...
	`)
			...
			Expect(err).ToNot(HaveOccurred(), "buildprobe file should be valid")
			Expect(buildprobeData.Image.Pullspec).To(Equal(outputRef))
			Expect(buildprobeData.BaseImage).To(BeZero())
			Expect(buildprobeData.BuilderBaseImages).To(BeEmpty())
